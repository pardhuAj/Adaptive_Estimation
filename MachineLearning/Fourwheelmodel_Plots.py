...
class fourwheel_model:
    ...
    def simulation(self,x, delta, ay):
        ...
        alpha11 = delta - np.atan((vy+self.lf*r)/(vx-(self.E*r/2)))
        alpha12 = delta - np.atan((vy+self.lf*r)/(vx+(self.E*r/2)))
        alpha21 = -np.atan((vy-self.lr*r)/(vx-(self.E*r/2)))
        alpha22 = -np.atan((vy-self.lr*r)/(vx+(self.E*r/2)))
